...
it("reads only the exact actor-owned asset and validates returned scope",async()=>{
  expect(await readOwnedNfcUploadIntent(asset,input.actor_id)).toEqual(row);
  expect(mocks.eq).toHaveBeenCalledWith("asset_id",asset);
  expect(mocks.eq).toHaveBeenCalledWith("actor_id",input.actor_id);
  mocks.maybeSingle.mockResolvedValue({data:{...row,actor_id:"50000000-0000-4000-8000-000000000001"},error:null});
  await expect(readOwnedNfcUploadIntent(asset,input.actor_id)).rejects.toThrow("NFC_UPLOAD_RESPONSE_INVALID");
});
it("distinguishes absent intent from failed reads",async()=>{
  mocks.maybeSingle.mockResolvedValue({data:null,error:null});
  expect(await readOwnedNfcUploadIntent(asset,input.actor_id)).toBeNull();
  mocks.maybeSingle.mockResolvedValue({data:null,error:{message:"private detail"}});
  await expect(readOwnedNfcUploadIntent(asset,input.actor_id)).rejects.toThrow("NFC_UPLOAD_READ_FAILED");
});
