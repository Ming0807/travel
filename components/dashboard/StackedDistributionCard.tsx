...
import { DASHBOARD_CHART_COLORS as COLORS } from "@/components/dashboard/dashboard-chart-theme";
      ...
      {total === 0 ? <div className="mt-4"><NoDataState description={emptyDescription} /></div> : (
        <>
          <div className="mt-5 flex h-3 overflow-hidden rounded-full bg-slate-100" role="img" aria-label={title}>
            {visible.map((item, index) => <span key={item.label} style={{ width: `${(item.value / total) * 100}%`, backgroundColor: COLORS[index % COLORS.length] }} />)}
          </div>
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {visible.map((item, index) => <li key={item.label} className="flex items-center justify-between gap-3 text-sm"><span className="flex min-w-0 items-center gap-2 text-slate-700"><span className="h-2.5 w-2.5 shrink-0 rounded-sm" style={{ backgroundColor: COLORS[index % COLORS.length] }} />{localizeDashboardLabel(item.label)}</span><strong className="shrink-0 tabular-nums text-slate-900">{Math.round((item.value / total) * 100)}%</strong></li>)}
          </ul>
