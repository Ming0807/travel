  ...
  return (
    <AdminShell>
      <div aria-busy="true" aria-label="กำลังโหลด Dashboard" className="animate-pulse space-y-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{Array.from({ length: 5 }, (_, index) => <div key={index} className="h-28 rounded-md bg-slate-100" />)}</div>
        <div className="grid gap-4 xl:grid-cols-2"><div className="h-72 rounded-md bg-slate-100" /><div className="h-72 rounded-md bg-slate-100" /></div>
      </div>
    </AdminShell>
  );
}
