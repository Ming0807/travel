...
const rows: NfcRecoveryReviewRow[] = states.map((status, index) => ({
  ...
  next_attempt_at: "2026-09-11T01:05:00Z", last_outcome: status === "review" ? "content_conflict" : null,
...
export async function getAdminNfcRecoveryAction(input: { page?: number }) {
  return { success: true as const, enabled: true as const, rows: input.page === 2 ? [] : rows, page: input.page ?? 1, pageSize: 20, hasMore: input.page !== 2 };
}
