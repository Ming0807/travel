...
import { asRecord, booleanValue, nullableBoolean, nullableNumber, nullableString, numberValue, stringValue } from "@/lib/utils/record";
...
function mapCheckinCode(rawRow: unknown): AdminCheckinCodeRow {
  ...
  const attraction = asRecord(firstJoin(row.attractions as { name_th?: unknown; is_active?: unknown; is_published?: unknown } | { name_th?: unknown; is_active?: unknown; is_published?: unknown }[] | null));
  ...
  return {
    ...
    campaign_id: nullableNumber(row.campaign_id),
    ...
    attraction_is_active: nullableBoolean(attraction.is_active),
    attraction_is_published: nullableBoolean(attraction.is_published),
