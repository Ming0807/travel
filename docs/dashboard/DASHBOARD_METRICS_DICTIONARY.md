...
For actions verified after migration `20260924000000_add_attraction_action_verification_snapshot.sql`, the verified transition atomically saves `attraction_improvement_actions.verification_snapshot` (JSONB version 1). It records the reviewed issue ID and snapshot version, attraction/dimension/metric, evidence scope, channel/campaign/check-in population, capture time, baseline and follow-up date windows, Visits and valid-response denominators, values, and a comparison state. Follow-up values are recomputed from `visits` and linked `satisfaction_surveys` using the saved population; raw tourist, visit, comment, and photo identifiers are excluded. Reads over the 5,000-Visit bound cannot be verified. Score actions created after this change must match the issue dimension, and the follow-up period must start after the baseline period. For score or low-score recurrence, at least 30 valid responses and 30 Visits are needed to display a follow-up value; coverage needs at least 30 Visits. A low-sample or unavailable value remains null, never zero-filled. Version-1 issue snapshots and historical verified actions without a verification snapshot are marked non-comparable. Two displayed values are a descriptive before/after observation, not a causal estimate. The snapshot is immutable after first write; updating the underlying operational data does not rewrite the recorded result.
The `structured_recurrence_count` is a raw count, not a rate. Even when both periods meet minimum sample thresholds, its snapshot comparison state is `descriptive_count`, not `comparable`: different window lengths and response bases can change the count without any change in underlying experience. Both denominators and date ranges remain visible; do not interpret a count difference as improvement.
Historical actions whose follow-up window overlaps the issue baseline receive `overlapping_period`, not a comparable before/after interpretation. New actions reject this overlap before creation.
