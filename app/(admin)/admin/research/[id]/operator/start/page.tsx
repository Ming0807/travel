    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-ink sm:py-12">
      <div className="mx-auto max-w-2xl">
        ...
        <form action={acceptResearchOperatorInvitationAction} className="mt-6 space-y-4">
          ...
          <label className="block border border-slate-300 bg-white p-4 text-sm font-bold">สำหรับผู้ดำเนินการ: รูปแบบการเก็บข้อมูล<select name="collectionMode" defaultValue={setup.studyKind === "pilot" ? "pilot_internal" : "field_observation"} className="mt-2 min-h-11 w-full border border-slate-300 bg-white px-3 font-normal">{setup.studyKind === "pilot" ? <><option value="pilot_internal">ทดสอบภายใน</option><option value="simulated_usability">สถานการณ์จำลอง</option></> : <option value="field_observation">ใช้งานจริง ณ สถานที่</option>}</select></label>
