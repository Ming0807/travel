...
const repository = vi.hoisted(() => ({ readAdminNfcTag: vi.fn(), updateAdminNfcTag: vi.fn(), insertAdminNfcTag: vi.fn(), listAdminNfcTags: vi.fn() }));
const guard = vi.hoisted(() => vi.fn());
vi.mock("@/lib/auth/guards", () => ({ requirePermission: guard }));
vi.mock("@/lib/repositories/admin-nfc.repository", () => repository);
import { changeNfcTag, createNfcTag, listNfcManagement } from "@/lib/services/admin-nfc.service";
