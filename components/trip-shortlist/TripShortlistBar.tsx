...
export function TripShortlistBar({
  itemLabel = "สถานที่ในทริป",
  actionLabel = "วางแผนจากรายการนี้",
  createHref = createTripPlanHref,
}: {
  itemLabel?: string;
  actionLabel?: string;
  createHref?: (slugs: readonly string[]) => string;
} = {}) {
  ...
  return (
    ...
    >
      ...
      <p className="min-w-0 flex-1 text-sm font-bold text-[var(--public-ink)]">{slugs.length.toLocaleString("th-TH")} {itemLabel}</p>
      <Link href={createHref(slugs)} className="inline-flex min-h-11 shrink-0 items-center gap-1 rounded-[var(--public-radius-control)] px-2 text-sm font-bold text-[var(--public-teal)]">
