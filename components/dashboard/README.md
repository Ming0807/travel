...
- `AttractionPeerComparison` compares an eligible attraction with at most three displayed active/published peers from the same province/type/date/evidence population. The selected attraction also needs at least 10 Visits; rank needs two eligible peers and a median needs three. Visit median uses all eligible peers; satisfaction median uses only peers with at least 10 valid score answers. It discloses the full eligible count and rank denominator, suppresses small survey cells, and keeps detailed dimensions behind progressive disclosure.
