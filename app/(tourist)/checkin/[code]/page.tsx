...
export default async function CheckinLandingPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F5] flex flex-col relative pb-24 overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-[55vh] bg-gradient-to-b from-ink/70 via-ink/30 to-[#FAF8F5] z-0">
        <div
          className="absolute inset-0 bg-cover bg-center mix-blend-overlay opacity-60 animate-scale-in"
          style={{ backgroundImage: `url('${heroImage}')`, animationDuration: '1.2s' }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#FAF8F5] via-[#FAF8F5]/90 to-transparent bottom-0 h-2/3 mt-auto" />
      </div>

      <div className="relative z-10 pt-[28vh] px-5 flex flex-col items-center max-w-lg mx-auto w-full">
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] p-8 w-full shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-white/50 text-center animate-fade-in-up delay-100">
          <div className="w-24 h-24 bg-gradient-to-br from-coral to-[#E05C3A] text-white rounded-[1.5rem] rotate-3 flex items-center justify-center mx-auto -mt-16 shadow-xl mb-6 border-[6px] border-white/90 backdrop-blur-md animate-scale-in delay-200 transition-transform hover:rotate-0 duration-300">
            <Camera size={44} weight="fill" className="-rotate-3" />
          </div>

          <h1 className="text-3xl font-black text-ink mb-3 leading-tight animate-fade-in-up delay-200">
            {photo_spot ? photo_spot.spot_name_th : attraction?.name_th}
          </h1>

          {photo_spot && attraction && (
            <p className="text-muted text-[15px] font-medium flex items-center justify-center gap-2 mb-8 animate-fade-in-up delay-300">
              <MapPin weight="fill" className="text-coral" />
              {attraction.name_th}
            </p>
          )}

          <div className="space-y-4 mb-10 text-left bg-white/60 p-6 rounded-[1.5rem] border border-ink/[0.03] shadow-sm animate-fade-in-up delay-400">
